...
import { tasks as defaultTasks, filterTasks } from '../data/desktopData';

export const StatusBadge = ({ task }) => <span className={`status-badge status-${task.statusColor}`}>{task.status}</span>;

export function TasksDesktopView({ filters, onFiltersChange, onSelectTask, tasks = defaultTasks, title = 'Công việc & Yêu cầu', scopeLabel = 'GreenCity Central' }) {
  ...
  return <div className="desktop-page">
    <div className="page-heading"><div><p className="eyebrow">Điều phối vận hành</p><h1>{title}</h1><p>{scopeLabel} · Theo dõi hạn xử lý và mở hồ sơ.</p></div><span className="subtle-badge">{tasks.length} công việc mẫu</span></div>
    <section className="surface task-workspace" aria-label="Danh sách công việc">
      <div className="status-filters" aria-label="Lọc trạng thái">
        {[...new Set(['all', 'Đang xử lý', 'Chờ duyệt', 'Quá hạn', ...tasks.map(task => task.status)])].map(value => <button key={value} onClick={() => set({ status: value })} aria-pressed={status === value} className={status === value ? 'is-selected' : ''}>{value === 'all' ? 'Tất cả' : value}<span>{value === 'all' ? tasks.length : tasks.filter(t => t.status === value).length}</span></button>)}
      </div>
      <div className="table-toolbar">
        ...
        <label className="inline-field"><span>Bộ phận</span><select value={department} onChange={e => set({ department: e.target.value })}><option value="all">Tất cả bộ phận</option>{[...new Set(tasks.map(task => task.department))].map(item => <option key={item}>{item}</option>)}</select></label>

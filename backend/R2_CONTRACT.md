...
- R2 không bao gồm frontend list/search UI, worker publish outbox, AI, resident portal,
  vệ sinh, an ninh, billing run, payment, refund hoặc chargeback.
...
| Scope/idempotency/link/triage | Các test còn lại trong `tests/test_r2_integration.py` |
...
Kết quả local không tự động chứng minh R1/Gate B đã đủ mọi AC tiền đề, không
chứng minh migration đã được áp dụng lên Aiven/production và không thay thế
review độc lập.

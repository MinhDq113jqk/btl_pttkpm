...
class Unit(IdentityTimestampMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("id", "building_id", name="uq_units_id_building_id"),
        Index("ix_greencity_units_building_id", "building_id"),
    )

    building_id: Mapped[UUID] = mapped_column(ForeignKey("greencity.buildings.id", ondelete="CASCADE"), nullable=False)
